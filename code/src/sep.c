...
#error "SEED_BYTES must be 32."
  ...
  // sample R from B_1 (binomial), reject if spectral norm exceeds bound
  kappa = 0;
  do {
...
#error "STATE_BYTES must be multiple of 4"
  ...
  *(uint32_t*)state = (uint32_t) stateinc;
  ...
  // compute tag inverse mod (b_H, x^n+1)
  poly_q_invert_mod_bH(taginv, sig->tag);

reject_signature:
  // sample v3 from discrete Gaussian with width s4
  poly_q_sample_gaussian_s4(sig->v3); // probabilistic

  // compute target: tmp = u + cmt - a3*v3
  poly_q_vec_d_mul_poly(tmp, a3, sig->v3);
  poly_q_vec_d_sub(tmp, cmt, tmp);
  poly_q_vec_d_add(tmp, u, tmp);

  // call tag-friendly sampler: output v11, sig->v12, sig->v2
  sampler(v11, sig->v12, sig->v2, sk->R, A, pk->B, tmp, sig->tag, taginv, sk->S); // probabilistic
...
int _sep_verify_from_commitment(const sep_sig_t *sig, const poly_q_vec_d cmt, const sep_pk_t *pk) {
  ...
  // compute v11 = u + cmt - A'.v12 + (B - q_L*b_H^i*tag*I).v2 - a3*v3
  ...
  bexpi = PARAM_QL;
  for (i = 0; i < PARAM_KH; i++) {
    // copy B to Btmp and subtract q_L * b_H^i * tag from diagonal
  ...
  // return check
  return (norm2sq_v11 <= PARAM_B11SQ) && (norm2sq_v12 <= PARAM_B12SQ) && (norm2sq_v2 <= PARAM_B2SQ) && (norm2sq_v3 <= PARAM_B3SQ) && (tag_weight == PARAM_W);
}
