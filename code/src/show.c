    ...
    const uint8_t msg[PARAM_M * PARAM_N / 8])
{
  size_t i, j;
  int64_t bexpi;
  uint64_t norm2sq_v11, norm2sq_v12, norm2sq_v2, norm2sq_v3, four_squares_res[4];
  poly_q tmp_poly;
  poly_q_mat_d_d A, Ds[2], Btmp;
  poly_q_mat_d_m D;
  poly_q_vec_d u, v11, a3;
  poly_q_vec_m m;
  ...
  for (i = 0; i < 4; i++)
  {
    poly_qshow_set_coeff(s1->entries[IDX_V2_FS], i, (coeff_qshow)(four_squares_res[i]));
  }
  // B3^2 - |v3|^2
  ...
  for (i = 0; i < 4; i++)
  {
    poly_qshow_set_coeff(s1->entries[IDX_V3_FS], i, (coeff_qshow)(four_squares_res[i]));
  }
  ...
  for (i = 0; i < PARAM_D; i++)
  {
    ...
    for (j = 0; j < PARAM_K_SHOW; j++)
    {
      poly_qshow_set(s1->entries[IDX_V12_SHOW + i * PARAM_K_SHOW + j], tmp_vec_k->entries[j]);
    }
  }
  // v2
  ...
  // bimodal bit slot (placeholder set to +1)
  ...
  for (i = 0; i < PARAM_D; i++)
  {
    ...
    poly_qshow_subring_embed_mat_k_k(A3_embed[i][0], a3->entries[i], PARAM_Q1_SHOW); // a3
    for (j = 0; j < PARAM_M; j++)
    {
